...
class AutoUsernameSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['aun_other_settings']['aun_max_length'] = array(
      ...
      '#min' => 1,
      '#max' => $max_length,
      '#description' => $this->t('Maximum length of aliases to generate. @max is the maximum possible length.', array('@max' => $max_length)),
    );
    $form['aun_other_settings']['aun_max_component_length'] = array(
      ...
      '#min' => 1,
      '#max' => $max_length,
      '#description' => $this->t('Maximum text length of any component in the username (e.g., [user:mail]). @max is the maximum possible length.', array('@max' => $max_length)),
    );
    ...
    foreach ($punctuation as $name => $details) {
      if ($details['value'] == $separator) {
        ...
        if ($action == AUN_PUNCTUATION_REMOVE) {
          drupal_set_message($this->t('You have configured the @name to be the separator and to be removed when encountered in strings. You should probably set the action for @name to be "replace by separator".', array('@name' => $details['name'])), 'error');
        }
      }
    }
  }
  ...
  public static function autoUsernamePatternprocessor($account) {
    ...
    if (trim($pattern)) {
      ...
      $output = $token_service->replace($pattern, array('user' => $account), array(
        'clear' => TRUE,
        'callback' => self::autoUsernameCleanTokenValues($pattern),
      ));
  ...
  public static function autoUsernameGenerateUsername(&$account) {
    ...
    $names = \Drupal::moduleHandler()->invokeAll('auto_username_name', array($account));
